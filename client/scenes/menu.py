"""Main menu: host a new game or join one by code."""
...
class MenuScene(Scene):
    def on_enter(self) -> None:
        cx = self.app.width // 2
        self.host_btn = ui.Button("HOST A GAME", (cx - 160, 300, 320, 52), self._host)
    ...
    def _host(self) -> None:
        self.app.net.send(protocol.C_CREATE_ROOM, name=self.app.name)
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        for w in (self.host_btn, self.code_in, self.join_btn):
            w.handle(event)
