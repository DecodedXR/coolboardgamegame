...
- [x] **Chunk 4 — Client components: wheel + shop UI + cutscene** — DONE: `client/wheel.py`
  (pure spin geometry `spin_angle`/`slice_at_pointer` round-tripping render-angle↔server
  `index`, the client never picks the slice; `slice_label`; `Wheel` widget), `client/shop_ui.py`
  (`ShopLayout`/`resolve_click` with affordability-gated `on_buy`, `on_skip`; `item_label`/blurb
